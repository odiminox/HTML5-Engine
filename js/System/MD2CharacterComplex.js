...
THREE.MD2CharacterComplex = function () {
	...
	// move via new position ** mikes stuff
	this.moveAngle = 0;
	this.moveToPos = 0;
	this.moving = false;
	...
	this.updateBehaviors = function ( delta ) {
		...
		if ( controls.moveForward || controls.moveBackward || controls.moveLeft || controls.moveRight || this.moving ) {
	...
	this.updateMovementModel = function ( delta ) {
		...
		// move to position updates
		if(this.moving == true){
		
			// get angle
			if(this.startMove == true){
				this.moveAngle = getMoveAngle(this.root.position, this.moveToPos);
				this.bodyOrientation = this.moveAngle;
				this.startMove = false;
			}
			...
			// end move
			if (this.root.position.x + (this.speed * delta) > this.moveToPos.x && this.root.position.x - (this.speed * delta) < this.moveToPos.x &&
				this.root.position.z + (this.speed * delta) > this.moveToPos.z && this.root.position.z - (this.speed * delta) < this.moveToPos.z){
				this.moving = false;		
			}
		...
		if ( ! ( controls.moveForward || controls.moveBackward || this.moving ) ) {
	...
	// get movement angle
	function getMoveAngle(start, end){
		var angle
			
		if (end.x > start.x && end.z > start.z){
			angle = Math.atan((end.x - start.x)/(end.z - start.z));
		}
		else if (end.x > start.x && end.z < start.z){
			angle = Math.atan((start.z - end.z)/(end.x - start.x));
			angle += Math.PI / 2;
		}			
		else if (end.x < start.x && end.z < start.z){
			angle = Math.atan((start.x - end.x)/(start.z - end.z));
			angle += Math.PI;
		}
		else if (end.x < start.x && end.z > start.z){
			angle = Math.atan((end.z - start.z)/(start.x - end.x));
			angle += ((Math.PI * 3) / 2) ;
		}
